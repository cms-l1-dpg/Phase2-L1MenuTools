...
def test_isolation_barrel():
    ...
    testargs = ['foo', "menu_tools/object_performance/tests/reference_data/ElectronsIsolation_Barrel_-999_V29.yaml"]
    ...
    with open("outputs/object_performance/V29/turnons/ElectronsIsolation_Barrel_-999_V29.json", "r") as f:
        test_result = json.load(f)
    with open("menu_tools/object_performance/tests/reference_data/ElectronsIsolation_Barrel_-999_V29.json", "r") as f:
        reference_data = json.load(f)

    for key, val in reference_data.items():
        if isinstance(val, dict):
            efficiencies_test = np.array(test_result[key]["efficiency"], dtype=np.float64)
